...
PCListLayer* PCListLayer::create() {
    auto ret = new PCListLayer();
    if (ret->init()) {
        ret->autorelease();
        return ret;
    }
...
CCScene* PCListLayer::scene() {
    auto ret = CCScene::create();
    AppDelegate::get()->m_runningScene = ret;
    ret->addChild(PCListLayer::create());
    return ret;
}

constexpr std::string_view pointercrateInfo =
    "<cg>Pointercrate</c> is the <cp>community-run</c> ranking of the hardest <cj>Geometry Dash</c> "
    "<cr>demons</c>, listing every verified extreme demon in the game.";

static GJSearchObject* getSearchObject(auto begin, auto end) {
    auto obj = GJSearchObject::create(SearchType::Type19);
    geode::utils::StringBuffer searchQuery;
    for (auto it = begin; it != end; ++it) {
        if (it != begin) searchQuery.append(',');
        searchQuery.append(*it);
    }
    obj->m_searchQuery = searchQuery.str();
    return obj;
}

static const char* getSearchKey(GJSearchObject* object) {
    std::string_view key = object->getKey();
    return key.data() + (key.size() > 256uz ? key.size() - 256uz : 0uz);
}

bool PCListLayer::init() {
    ...
    m_list = GJListLayer::create(nullptr, "Pointercrate Demon List", { 0, 0, 0, 180 }, 356.0f, 220.0f, 0);
    ...
    m_searchButton = CCMenuItemSpriteExtra::create(searchSprite, this, menu_selector(PCListLayer::onSearch));
    ...
    auto backButton = CCMenuItemSpriteExtra::create(
        CCSprite::createWithSpriteFrameName("GJ_arrow_01_001.png"), this, menu_selector(PCListLayer::onBack)
    );
    ...
    m_leftButton = CCMenuItemSpriteExtra::create(leftBtnSpr, this, menu_selector(PCListLayer::onPrevPage));
    ...
    m_rightButton = CCMenuItemSpriteExtra::create(rightBtnSpr, this, menu_selector(PCListLayer::onNextPage));
    ...
    m_infoButton = InfoAlertButton::create("Pointercrate Demon List", gd::string(pointercrateInfo.data(), pointercrateInfo.size()), 1.0f);
    ...
    auto refreshButton = CCMenuItemSpriteExtra::create(refreshBtnSpr, this, menu_selector(PCListLayer::onRefresh));
    ...
    m_pageButton = CCMenuItemSpriteExtra::create(pageBtnSpr, this, menu_selector(PCListLayer::onPage));
    ...
    m_randomButton = CCMenuItemSpriteExtra::create(randomSprite, this, menu_selector(PCListLayer::onRandom));
    ...
    m_lastButton = CCMenuItemSpriteExtra::create(lastArrow, this, menu_selector(PCListLayer::onLast));
    ...
    m_firstButton = CCMenuItemSpriteExtra::create(firstArrow, this, menu_selector(PCListLayer::onFirst));
    ...
    if (IntegratedPointercrate::demonsLoaded) {
        populateList("");
    }
    else {
...
void PCListLayer::onBack(CCObject* sender) {
    CCDirector::get()->popSceneWithTransition(0.5f, kPopTransitionFade);
}

void PCListLayer::onPrevPage(CCObject* sender) {
    page(m_page - 1);
}

void PCListLayer::onNextPage(CCObject* sender) {
    page(m_page + 1);
}
...
void PCListLayer::populateList(const std::string& query) {
    m_fullSearchResults.clear();
    auto searchSprite = static_cast<CCSprite*>(m_searchButton->getNormalImage());
    if (query.empty()) {
        for (auto& demon : IntegratedPointercrate::demons) {
            m_fullSearchResults.push_back(fmt::to_string(demon.id));
        }
        ...
        for (auto& demon : IntegratedPointercrate::demons) {
            if (!string::toLower(demon.name).contains(lowerQuery)) continue;
            m_fullSearchResults.push_back(fmt::to_string(demon.id));
        }
    ...
    if (m_fullSearchResults.empty()) {
        ...
        auto searchObject = getSearchObject(
            m_fullSearchResults.begin() + m_page * 10,
            std::min(m_fullSearchResults.end(), m_fullSearchResults.begin() + (m_page + 1) * 10)
        );

        if (auto storedLevels = glm->getStoredOnlineLevels(getSearchKey(searchObject))) {
...
void PCListLayer::loadLevelsFinished(CCArray* levels, const char*, int) {
...
void PCListLayer::loadLevelsFailed(const char*, int) {
    m_searchBarMenu->setVisible(true);
    m_countLabel->setVisible(true);
    m_loadingCircle->setVisible(false);
...
void PCListLayer::setupPageInfo(gd::string, const char*) {
...
void PCListLayer::onSearch(CCObject* sender) {
...
void PCListLayer::page(int page) {
...
void PCListLayer::keyDown(enumKeyCodes key, double timestamp) {
    switch (key) {
        case KEY_Left:
...
void PCListLayer::keyBackClicked() {
    onBack(nullptr);
}

void PCListLayer::setIDPopupClosed(SetIDPopup*, int page) {
    auto maxPage = m_fullSearchResults.empty() ? 0 : (m_fullSearchResults.size() - 1) / 10;
    m_page = std::clamp<int>(page - 1, 0, static_cast<int>(maxPage));
    showLoading();
    populateList(m_query);
}

PCListLayer::~PCListLayer() {
    auto glm = GameLevelManager::get();
